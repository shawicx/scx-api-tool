...
export class RequestError extends Error {
  constructor(
    ...
    public message: any,
    public httpStatusOrBusinessCode = 0,
  ) {
...
export default async function request<TResponseData>(
  ...
  try {
    ...
    const fetchOptions: RequestInit = {
      ...
      headers: {
        ...(payload.hasFileData
          ? {}
          : (() => {
              if (payload.requestBodyType === RequestBodyType.json) {
    ...
    } else if ((fetchRes.headers.get('Content-Type') || '').indexOf('application/json') >= 0) {
      res = await fetchRes
        .json()
    ...
    if (res != null && typeof res === 'object' && res.code != null && res.code !== 0) {
