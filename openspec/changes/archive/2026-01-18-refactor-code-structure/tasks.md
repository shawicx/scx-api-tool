...
- [x] 7.1 创建 `src/processors/common.ts`
  - 识别 TypeScript 类型和 Zod Schema 生成的公共逻辑
  - 提取路径处理逻辑
  - 提取标签处理逻辑
  - 为所有函数添加 JSDoc 注释

- [x] 7.2 更新 `src/processors/openapi.ts`
...
- [x] 8.2 添加核心函数的 `@example` 标签
  - 识别关键函数（命名策略、类型清理、生成器等）
  - 为每个函数添加 `@example` 标签和示例代码

- [x] 8.3 补充 `@param` 和 `@returns` 标签
  - 遍历所有函数
  - 确保每个参数有 `@param` 注释
...
1. ✅ 所有新文件行数 <360（注：`src/generator/template/compiler.ts` 例外，因为主要包含模板字符串）
2. ✅ 所有函数有 JSDoc 注释
...
| 阶段 5：命名策略模块创建    | 2 小时      |
| 阶段 6：生成器模块拆分      | 3 小时      |
| 阶段 7：公共逻辑抽离        | 1 小时      | ✅ 已完成 |
| 阶段 8：注释风格统一        | 2 小时      | ✅ 已完成 |
| 阶段 9：最终验证和测试      | 1.5 小时    | ✅ 已完成 |
| 阶段 10：文档更新和清理     | 0.5 小时    | ✅ 已完成 |
| **总计**                    | **16 小时** |
