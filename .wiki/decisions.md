...
- `concurrency` 设置（默认值：50）限制并行文件写入数量
...
**背景**：OpenAPI 规范中的标签可能使用中文字符（如"用户管理"、"权限管理"），直接作为目录名会导致文件系统兼容性问题。

**决策**：使用 `pinyin-pro` 将中文标签自动转换为驼峰拼音目录名。

**理由**：

- `chineseToPinyinCamelCase()` 函数将中文标签转为拼音驼峰（如"用户管理" → `YongHuGuanLi`）
- 保证生成目录名在所有文件系统上兼容
- 拼音保留了语义可读性，便于开发者定位
- 非中文字符保持原样
...
- 新增 `pinyin-pro` 运行时依赖
- 中文标签目录名与原始标签名差异较大，需要依赖 root `index.ts` 中的导出进行关联
...
**背景**：CLI 工具在配置加载、数据获取、代码生成等阶段都可能出错，需要统一的错误处理机制，为用户提供清晰的错误信息和修复建议。

**决策**：实现三层错误类体系 + ErrorFactory 工厂模式。

**理由**：

- `BaseError` 提供通用错误格式化（错误码、消息、解决方案、原始错误）
- 三大子类：`ConfigError`（E1xxx）、`FetchError`（E2xxx）、`GenerateError`（E3xxx）
- `ErrorFactory` 提供语义化的工厂方法（如 `configNotFound()`、`unauthorized()`）
- 每种错误附带可操作的解决方案建议
- `handleError()` + `withErrorHandling()` 统一错误处理入口

**影响**：

- 所有模块通过 `ErrorFactory` 创建错误，而非直接 `throw new Error()`
- 错误码体系（E1xxx-E3xxx）需要随新错误场景扩展
