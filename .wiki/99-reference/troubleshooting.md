...
- 编辑器**原子保存**（写临时文件后 rename）可能使 `fs.watch` 丢失对该文件的跟踪，此时重启 watch 进程

> watch 已内置防抖（300ms 合并一次保存的多个事件）、生成互斥（进行中只排队一次重跑）与配置缓存清理（重新生成前 `clearConfigCache` 并穿透 ESM 模块缓存），正常场景下配置修改会即时生效。

### 部分服务获取失败（多服务场景）

**现象**：`服务获取失败，已跳过: <source>` + `部分服务获取失败（成功 N/M 个）`，进程以非零码退出。

**说明**：这是**失败隔离**的预期行为——单个服务数据源不可达只跳过该服务，其余服务照常完成生成；修复失败服务后重新运行即可。检查该服务的 `source` 可达性与 `token` 有效性。
